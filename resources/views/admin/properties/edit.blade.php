    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.properties.update", [$property->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" type="text" name="address" id="address" value="{{ old('address', $property->address) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rooms') ? 'is-invalid' : '' }}" type="number" name="rooms" id="rooms" value="{{ old('rooms', $property->rooms) }}" step="1">
                @if($errors->has('rooms'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bathrooms') ? 'is-invalid' : '' }}" type="number" name="bathrooms" id="bathrooms" value="{{ old('bathrooms', $property->bathrooms) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('parking') ? 'is-invalid' : '' }}" name="parking" id="parking">
                    ...
                    @foreach(App\Models\Property::PARKING_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('parking', $property->parking) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $property->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
